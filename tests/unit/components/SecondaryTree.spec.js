...
//   getPartialEntity: vi.fn().mockResolvedValue(ENTITY),
...
describe("SecondaryTree.vue", () => {
  ...
  it("tree has parent and selected item", async () => {
    getPartialEntitySpy = vi.spyOn(EntityService.prototype, "getPartialEntity").mockResolvedValue(ENTITY);
    ...
    //     if (iri === "http://snomed.info/sct#111266001") {
    ...
    //     const iri = req.url.searchParams.get("iri");
    //     if (iri === "http://snomed.info/sct#111266001") {
    //       return res(ctx.status(200), ctx.json(PARENTS));
    //     } else
    //       return res(
    //         ctx.status(500),
    //         ctx.json({
    ...
    //     const iri = req.url.searchParams.get("iri");
    //     if (iri === "http://snomed.info/sct#111266001") {
    //       return res(ctx.status(200), ctx.json(CHILDREN));
    //     } else
    //       return res(
    //         ctx.status(500),
    //         ctx.json({
    ...
    const component = render(SecondaryTree, {
      global: {
    ...
    const parent = component.getByText("Acquired scoliosis (disorder)");
    expect(parent).toBeTruthy();
    // expect(parent.).toBe("Acquired scoliosis (disorder)");
  });
});
