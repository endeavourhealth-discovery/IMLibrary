...
- In the root folder of your client app, run `npm link im-library`. This will add the symbolic link to the `node_modules` folder in your client app.
- You can now import `im-library` in your client app.

There is no need to add `im-library` to your client app's dependency in this case.
...
```js
<script lang="ts">
import { MyConstants, MyUtil } from 'im-library'
